...
public class CRFSuiteWrapper {
    ...
    public CRFSuiteWrapper() throws Exception {
        ...
        if (exec.isInstalled()) {
            iv_logger.log(Level.INFO, "The CRFSuite is installed on the system");
            ...
            if (!exec.isInstalled(this.executable.getAbsolutePath())) {
                iv_logger.log(Level.WARNING,
                        "The CRFSuite binary is not available for the current operation system, please install it!");
            } else {
                iv_logger.log(Level.INFO, "The CRFSuite binary is successfully extracted");
            }
        }
    }
    ...
    class Executables {
        ...
        public boolean isInstalled(String path) throws Exception {
            ...
            try {
                ...
                if (str.length() < 8) {
                     iv_logger.log(Level.WARNING, "CRFSuite could not be executed!");
                }
                return str.length() >= 8 && str.substring(0, 8).equals("CRFSuite");
            } catch (IOException e) {
                 iv_logger.log(Level.WARNING, e.getMessage());
            }
            return false;
        }
        ...
        public File getExecutable() {
            ...
            crfExecUrl = ClassLoader.getSystemResource(loc);
             iv_logger.log(Level.INFO, "CrfSuite Location " + loc);
             iv_logger.log(Level.INFO, "CrfSuite Url: " + crfExecUrl);
            File f;
            try {
                if (crfExecUrl != null) {
                 ...
                 iv_logger.log(Level.WARNING, "The executable could not be found at " + loc);
                return null;
            } catch (IOException e) {
                e.printStackTrace();
    ...
    public void trainClassifier(String model, HashMap<List<Feature>, String> featureslabel, String[] args)
        ...
        featureFile.deleteOnExit();
         iv_logger.log(Level.INFO, "Write features to classify to " + featureFile.getAbsolutePath());
